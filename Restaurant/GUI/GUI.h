...
private:
	...
	const Order * OrdListForDrawing[MaxPossibleOrdCnt]; // This Array of Pointers is used for drawing elements in the GUI
	//NOTES: 
	...
	void DrawSingleOrder(const Order* pO, int RegionCount) const;		//draws ONE order 
...
public:
	...
	// Output Functions  ---------------------------
	void PrintMessage(string msg,int line=0) const; // prints a message in the status bar


	void UpdateInterface();
	void AddOrderForDrawing(const Order* pOrd);	//Adds a new order to the drawing list
